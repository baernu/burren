...
//            People user = repository.findEmail(request.getAttribute("email").toString());
            String tokenString = "aberja"; //token.generateJWT(user.getUserID(), user.getType(), user.getFirstName(), user.getLastName(), user.getEmail());
            if(tokenString == null) {
//                connection.rollback();
                response.setStatus(SC_UNAUTHORIZED);
                objectMapper.writeValue(response.getOutputStream(), "Token invalid");
            }
